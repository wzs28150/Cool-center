...
class User extends Common
{
    ...
    {
			// dump($this->belongsToMany('group', 'admin_access', 'group_id', 'user_id'));
    ...
    public function getDataList($keywords, $page, $limit)
    {
        ...
        $list = $this->where($map)->alias('user')->join('__ADMIN_STRUCTURE__ structure', 'structure.id=user.structure_id', 'LEFT')->join('__ADMIN_POST__ post', 'post.id=user.post_id', 'LEFT');
        // 若有分页
        if ($page && $limit) {
            $list = $list->page($page, $limit);
        }
        $list = $list->field('user.*,structure.name as s_name, post.name as p_name')->select();
        foreach ($list as $key => $value) {
          $garr = $this->get($value['id'])->groups;
          $list[$key]['groups'] = $garr[0]['title'];
        }
        $data['list'] = $list;
